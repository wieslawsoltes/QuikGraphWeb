...
import { optionalRuntimeChecks } from './package-runtimes.mjs';
...
function run(command, argv, cwd, label) {
  ...
  const result = spawnSync(command, argv, { cwd, encoding: 'utf8', timeout: 120_000, env });
  assert.equal(result.status, 0, `${label}:\n${result.stdout ?? ''}${result.stderr ?? ''}${result.error ?? ''}`);
  return result.stdout;
}
await mkdir('test-results', { recursive: true });
const temporary = await mkdtemp(join(project, 'test-results/package-'));
// Keep consumers outside the project so unbundled imports cannot accidentally
// resolve upward into this repository's development node_modules directory.
const consumer = await mkdtemp(join(tmpdir(), 'quikgraphweb-installed-'));
try {
  ...
  const installed = JSON.parse(await readFile(join(consumer, 'node_modules', pkg.name, 'package.json'), 'utf8'));
  const runtimes = optionalRuntimeChecks.filter(check => Object.hasOwn(installed.exports, './' + check.name));
  for (const check of runtimes) {
    const entry = installed.exports['./' + check.name];
    for (const condition of ['types', 'import', 'require']) {
      assert.equal(typeof entry[condition], 'string', `${check.name} lacks ${condition} export`);
      assert(files.has(entry[condition].replace(/^\.\//, '')), `${check.name} ${condition} target is missing from the tarball`);
    }
    for (const file of [check.browser, ...(check.assets ?? [])].filter(Boolean)) assert(files.has(file), `Tarball lacks optional runtime asset ${file}`);
  }
  await writeFile(join(consumer, 'isolation.mjs'), `import assert from 'node:assert/strict';\nimport { createRequire } from 'node:module';\nconst require = createRequire(import.meta.url);\nfor (const dependency of ['@viz-js/viz', '@msagl/core', 'libxml2-wasm', 'esbuild', 'typescript']) {\n  assert.throws(() => require.resolve(dependency), { code: 'MODULE_NOT_FOUND' }, 'Consumer must not resolve development dependency ' + dependency);\n  assert.throws(() => import.meta.resolve(dependency), { code: 'ERR_MODULE_NOT_FOUND' }, 'Consumer must not resolve development ESM dependency ' + dependency);\n}\nconsole.log('Consumer has no access to project development dependencies: passed');\n`);
  process.stdout.write(run(process.execPath, ['isolation.mjs'], consumer, 'development dependency isolation'));
  const exercise = `
...
`;
  ...
  const optionalTypeFiles = [];
  const hasViewerLayout = run(process.execPath, ['-e', `console.log(typeof require(${JSON.stringify(pkg.name)}).QuikGraphViewer.prototype.LayoutAsync === 'function')`], consumer, 'viewer layout capability').trim() === 'true';
  for (const check of runtimes) {
    const specifier = JSON.stringify(pkg.name + '/' + check.name);
    const esm = `import assert from 'node:assert/strict';\nimport * as core from ${JSON.stringify(pkg.name)};\nimport * as runtime from ${specifier};\n${exercise}\n${check.runtime}\nconsole.log('Installed ESM ${check.name}: passed');\n`;
    const cjs = `const assert = require('node:assert/strict');\nconst core = require(${JSON.stringify(pkg.name)});\nconst runtime = require(${specifier});\n(async () => {\n${exercise}\n${check.runtime}\nconsole.log('Installed CommonJS ${check.name}: passed');\n})().catch(error => { console.error(error); process.exitCode = 1; });\n`;
    await writeFile(join(consumer, check.name + '.mjs'), esm);
    await writeFile(join(consumer, check.name + '.cjs'), cjs);
    process.stdout.write(run(process.execPath, [check.name + '.mjs'], consumer, `ESM ${check.name} consumer`));
    process.stdout.write(run(process.execPath, [check.name + '.cjs'], consumer, `CommonJS ${check.name} consumer`));
    for (const [extension, imports] of [
      ['ts', `import * as core from ${JSON.stringify(pkg.name)};\nimport * as runtime from ${specifier};`],
      ['cts', `import core = require(${JSON.stringify(pkg.name)});\nimport runtime = require(${specifier});`],
    ]) {
      const file = check.name + '-consumer.' + extension;
      await writeFile(join(consumer, file), `${imports}\nasync function consume() {\nconst graph = new core.BidirectionalGraph<string, core.Edge<string>>();\ngraph.AddVerticesAndEdge(new core.Edge('A', 'B'));\n${check.types}\n${hasViewerLayout ? check.viewerTypes ?? '' : ''}\n}\nvoid consume;\n`);
      optionalTypeFiles.push(file);
    }
  }
  ...
  run(process.execPath, [join(project, 'node_modules/typescript/bin/tsc'), '--ignoreConfig', '--noEmit', '--strict', '--target', 'ES2022', '--module', 'NodeNext', '--moduleResolution', 'NodeNext', '--lib', 'ES2022,DOM', 'consumer.ts', 'consumer.cts', 'contracts.ts', 'equality.ts', ...optionalTypeFiles], consumer, 'TypeScript ESM and CommonJS consumers');
  console.log(`Verified exact ${pkg.name}@${pkg.version} tarball (${files.size} files), exports, declarations, graph algorithms, browser bundles and ${runtimes.length} optional runtimes without development dependencies.`);
} finally {
  await Promise.all([rm(temporary, { recursive: true, force: true }), rm(consumer, { recursive: true, force: true })]);
}
