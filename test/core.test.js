...
// Remaining upstream graph constructor overload cases.
for(const name of ['BidirectionalMatrixGraph','DelegateBidirectionalIncidenceGraph','CompressedSparseRowGraph','UndirectedBidirectionalGraph','DelegateImplicitUndirectedGraph','DelegateIncidenceGraph','DelegateImplicitGraph','BidirectionalAdapterGraph','ClusteredAdjacencyGraph','DelegateUndirectedGraph','EdgeListGraph','ReversedBidirectionalGraph','DelegateVertexAndEdgeListGraph','FilteredImplicitVertexSetGraph'])test(`${name}Tests.Construction`,()=>{
 const g=validationGraph(name+'Tests');assert.equal(g.IsDirected,!name.includes('Undirected'));assert.equal(g.AllowParallelEdges,name!=='BidirectionalMatrixGraph'&&name!=='CompressedSparseRowGraph');
 if(name==='BidirectionalMatrixGraph'){for(const n of [0,1,12]){const m=new Q.BidirectionalMatrixGraph(n);assert.equal(m.VertexCount,n);assert.equal(m.EdgeCount,0);}}
 else if(name==='EdgeListGraph'){for(const d of [false,true])for(const p of [false,true]){const m=new Q.EdgeListGraph(d,p);assert.equal(m.IsDirected,d);assert.equal(m.AllowParallelEdges,p);assert.equal(m.EdgeCount,0);}}
 else if(name.startsWith('Delegate')){assert(g.ContainsVertex(1));assert.deepEqual(g.OutEdges(1),[]);assert.equal(g.TryGetOutEdges(99),undefined);}
 else {assert.equal(g.VertexCount,2);assert.equal(g.EdgeCount,0);}
});
test('ClusteredAdjacencyGraphTests.Collapsed',()=>{const g=new Q.ClusteredAdjacencyGraph(new Q.AdjacencyGraph());assert.equal(g.Collapsed,false);g.Collapsed=true;assert.equal(g.Collapsed,true);g.Collapsed=false;assert.equal(g.Collapsed,false);});
test('ClusteredAdjacencyGraphTests.AddCluster',()=>{const g=new Q.ClusteredAdjacencyGraph(new Q.AdjacencyGraph()),a=g.AddCluster(),b=g.AddCluster();assert.equal(g.ClustersCount,2);assert.equal(a.Parent,g);assert.equal(b.Parent,g);a.AddVertex(1);assert(g.ContainsVertex(1));assert(!b.ContainsVertex(1));});
test('ClusteredAdjacencyGraphTests.RemoveCluster',()=>{const g=new Q.ClusteredAdjacencyGraph(new Q.AdjacencyGraph()),a=g.AddCluster();a.AddVertex(1);g.RemoveCluster(a);assert.equal(g.ClustersCount,0);assert(g.ContainsVertex(1));assert(a.ContainsVertex(1));});
test('BidirectionalAdapterGraphTests.AdapterLimits',()=>{const g=new Q.AdjacencyGraph();g.AddVertexRange([1,2]);const a=new Q.BidirectionalAdapterGraph(g),e=new Q.Edge(1,2);g.AddEdge(e);assert.equal(a.EdgeCount,1);assert.equal(a.OutDegree(1),1);assert.equal(a.InDegree(2),0);g.AddVertex(3);assert(a.ContainsVertex(3));assert.throws(()=>a.InEdges(3),Q.VertexNotFoundException);});
for(const C of [Q.ArrayUndirectedGraph,Q.UndirectedBidirectionalGraph])test(`${C.name}Tests.ContainsEdge_Undirected`,()=>{const g=C===Q.ArrayUndirectedGraph?new Q.UndirectedGraph():new Q.BidirectionalGraph();g.AddVerticesAndEdge(new Q.UndirectedEdge(1,2));const a=new C(g);assert(a.ContainsEdge(2,1));assert(a.ContainsEdge(1,2));assert(!a.ContainsEdge(2,3));});
// Upstream BidirectionalGraphTests.Merge_Test/MergeIf_Test helper counters and exact fixtures.
function mergeFixture(vertices,pairs,predicate,expectedVertices,expectedPairs,added,removed){const g=new Q.BidirectionalGraph();g.AddVertexRange(vertices);g.AddEdgeRange(pairs.map(([s,t])=>new Q.EquatableEdge(s,t)));let va=0,vr=0,ea=0,er=0;g.VertexAdded.add(()=>++va);g.VertexRemoved.add(()=>++vr);g.EdgeAdded.add(()=>++ea);g.EdgeRemoved.add(()=>++er);if(typeof predicate==='function')g.MergeVerticesIf(predicate,(s,t)=>new Q.EquatableEdge(s,t));else g.MergeVertex(predicate,(s,t)=>new Q.EquatableEdge(s,t));assert.equal(va,0);assert.equal(vr,vertices.length-expectedVertices.length);assert.equal(ea,added);assert.equal(er,removed);assert.deepEqual(g.Vertices,expectedVertices);assert.deepEqual(g.Edges.map(e=>`${e.Source}:${e.Target}`).sort(),expectedPairs.map(p=>p.join(':')).sort());}
const mergePairs=[[1,3],[1,3],[2,1],[2,3],[3,4],[3,5],[3,5],[4,5]];
test('BidirectionalGraphTests.Merge1',()=>mergeFixture([1,2,3,4,5],mergePairs,3,[1,2,4,5],[[2,1],[4,5],[1,4],[1,5],[1,5],[1,4],[1,5],[1,5],[2,4],[2,5],[2,5]],9,6));
test('BidirectionalGraphTests.Merge2',()=>mergeFixture([1,2,3,4],[[2,3],[3,1],[3,3],[3,4]],3,[1,2,4],[[2,1],[2,4]],2,4));
test('BidirectionalGraphTests.Merge3',()=>mergeFixture([1,2,3,4],[[3,4]],1,[2,3,4],[[3,4]],0,0));
test('BidirectionalGraphTests.Merge_Test',()=>mergeFixture([1,2,3],[[1,2],[2,3]],2,[1,3],[[1,3]],1,2));
test('BidirectionalGraphTests.MergeIf1',()=>mergeFixture([1,2,3,4,5],mergePairs,v=>v===3||v===4,[1,2,5],[[2,1],...[1,1,1,1,2,2,1,1,2].map(v=>[v,5])],12,10));
test('BidirectionalGraphTests.MergeIf2',()=>mergeFixture([1,2,3,4],[[2,3],[3,1],[3,3],[3,4]],v=>v===3||v===4,[1,2],[[2,1]],2,5));
test('BidirectionalGraphTests.MergeIf3',()=>mergeFixture([1,2,3,4],[[3,4]],v=>v===1||v===2,[3,4],[[3,4]],0,0));
test('BidirectionalGraphTests.MergeIf4',()=>mergeFixture([1,2,3,4],[[3,4]],v=>v===1||v===3,[2,4],[],0,1));
test('BidirectionalGraphTests.MergeIf_Test',()=>mergeFixture([1,2,3],[[1,2],[2,3]],v=>v===2,[1,3],[[1,3]],1,2));
// Upstream predicate construction/null-key fixtures.
for(const C of [Q.IsolatedVertexPredicate,Q.SinkVertexPredicate,Q.InDictionaryVertexPredicate,Q.ResidualEdgePredicate,Q.ReversedResidualEdgePredicate]){
 test(`${C.name}Tests.Construction`,()=>{const graph=new Q.BidirectionalGraph(),map=new Map(),reverse=new Map(),p=C===Q.ReversedResidualEdgePredicate?new C(map,reverse):new C(C===Q.IsolatedVertexPredicate||C===Q.SinkVertexPredicate?graph:map);if(C===Q.ResidualEdgePredicate||C===Q.ReversedResidualEdgePredicate)assert.equal(p.ResidualCapacities,map);else assert(p);if(C===Q.ReversedResidualEdgePredicate)assert.equal(p.ReversedEdges,reverse);});
 if(![Q.IsolatedVertexPredicate,Q.SinkVertexPredicate].includes(C)){
 test(`${C.name}Tests.Construction_Throws`,()=>{assert.throws(()=>new C(null));if(C===Q.ReversedResidualEdgePredicate)assert.throws(()=>new C(new Map(),null));});
 test(`${C.name}Tests.Predicate_Throws`,()=>{const p=C===Q.ReversedResidualEdgePredicate?new C(new Map(),new Map()):new C(new Map());assert.throws(()=>p.Test(null));if(C!==Q.InDictionaryVertexPredicate)assert.throws(()=>p.Test(new Q.Edge(1,2)));});
 }
}
// Upstream GraphExtensionsTests delegate overloads, adapted TryFunc => iterable/undefined.
for(const suffix of ['TryGetDelegate','GetDelegate']){
 test(`GraphExtensionsTests.ToDelegateIncidenceGraph_${suffix}`,()=>{const e=new Q.Edge(1,2),m=new Map([[1,[e]],[2,[]]]),g=Q.ToDelegateIncidenceGraph(v=>m.get(v));assert.equal(g.TryGetEdge(1,2),e);assert.deepEqual(g.OutEdges(2),[]);assert(!g.ContainsVertex(99));});
 test(`GraphExtensionsTests.ToDelegateIncidenceGraph_${suffix}_Throws`,()=>assert.throws(()=>Q.ToDelegateIncidenceGraph(null)));
 test(`GraphExtensionsTests.ToDelegateVertexAndEdgeListGraph_${suffix}`,()=>{const e=new Q.Edge(1,2),g=Q.ToDelegateVertexAndEdgeListGraph([1,2],v=>v===1?[e]:[]);assert.equal(g.VertexCount,2);assert.equal(g.EdgeCount,1);assert(g.ContainsEdge(e));});
 test(`GraphExtensionsTests.ToDelegateVertexAndEdgeListGraph_${suffix}_Throws`,()=>{assert.throws(()=>Q.ToDelegateVertexAndEdgeListGraph(null,()=>[]));assert.throws(()=>Q.ToDelegateVertexAndEdgeListGraph([],null));});
 test(`GraphExtensionsTests.ToDelegateUndirectedGraph_${suffix}`,()=>{const e=new Q.Edge(1,2),g=Q.ToDelegateUndirectedGraph([1,2],()=>[e]);assert.equal(g.EdgeCount,1);assert(g.ContainsEdge(1,2));assert(g.ContainsEdge(2,1));});
 test(`GraphExtensionsTests.ToDelegateUndirectedGraph_${suffix}_Throws`,()=>{assert.throws(()=>Q.ToDelegateUndirectedGraph(null,()=>[]));assert.throws(()=>Q.ToDelegateUndirectedGraph([],null));});
}
test('GraphExtensionsTests.ToDelegateVertexAndEdgeListGraph',()=>{const e=new Q.Edge(1,2),m=new Map([[1,[e]],[2,[]]]),g=Q.ToDelegateVertexAndEdgeListGraph(m);assert.equal(g.EdgeCount,1);assert.equal(g.TryGetEdge(1,2),e);});
test('GraphExtensionsTests.ToDelegateVertexAndEdgeListGraph_Throws',()=>assert.throws(()=>Q.ToDelegateVertexAndEdgeListGraph(null)));
test('GraphExtensionsTests.ToDelegateVertexAndEdgeListGraph_ConverterEdges_Throws',()=>{assert.throws(()=>Q.ToDelegateVertexAndEdgeListGraph(null,()=>[]));assert.throws(()=>Q.ToDelegateVertexAndEdgeListGraph(new Map(),null));});
test('GraphExtensionsTests.ToDelegateBidirectionalIncidenceGraph',()=>{const e=new Q.Edge(1,2),g=Q.ToDelegateBidirectionalIncidenceGraph(v=>v===1?[e]:[],v=>v===2?[e]:[]);assert.equal(g.OutEdge(1,0),e);assert.equal(g.InEdge(2,0),e);assert.equal(g.Degree(1),1);});
test('GraphExtensionsTests.ToDelegateBidirectionalIncidenceGraph_Throws',()=>{assert.throws(()=>Q.ToDelegateBidirectionalIncidenceGraph(null,()=>[]));assert.throws(()=>Q.ToDelegateBidirectionalIncidenceGraph(()=>[],null));});
for(const name of ['ToAdjacencyGraph','ToBidirectionalGraph'])for(const suffix of ['EdgeSet','VertexPairs','EdgeSetWithFactory'])test(`GraphExtensionsTests.${name}_${suffix}_Throws`,()=>{assert.throws(()=>Q[name](null));if(suffix==='EdgeSetWithFactory')assert.throws(()=>Q[name]([1,2],null));else assert.throws(()=>Q[name]([null]));});
for(const name of ['ToArrayAdjacencyGraph','ToArrayBidirectionalGraph','ToArrayUndirectedGraph','ToCompressedRowGraph']){
 test(`GraphExtensionsTests.${name}`,()=>{const g=name==='ToArrayUndirectedGraph'?new Q.UndirectedGraph():new Q.BidirectionalGraph();g.AddVerticesAndEdge(new Q.Edge(1,2));g.AddVertex(3);const a=Q[name](g);assert.equal(a.VertexCount,3);assert.equal(a.EdgeCount,1);assert(a.ContainsEdge(1,2));g.Clear();assert.equal(a.VertexCount,3);});
 test(`GraphExtensionsTests.${name}_Throws`,()=>assert.throws(()=>Q[name](null)));
}
for(const suffix of ['FromDirectedGraph','FromUndirectedGraph'])test(`GraphExtensionsTests.ToBidirectionalGraph_${suffix}_Throws`,()=>assert.throws(()=>Q.ToBidirectionalGraph(null)));
test('GraphExtensionsTests.ToUndirectedGraph',()=>{const g=Q.ToUndirectedGraph([new Q.Edge(1,2),new Q.Edge(2,3)]);assert(!g.IsDirected);assert(g.ContainsEdge(2,1));assert.equal(g.EdgeCount,2);});
test('GraphExtensionsTests.ToUndirectedGraph_Throws',()=>{assert.throws(()=>Q.ToUndirectedGraph(null));assert.throws(()=>Q.ToUndirectedGraph([null]));});
test('GraphExtensionsTests.ToUndirectedGraph_VertexPairs',()=>{const g=Q.ToUndirectedGraph([new Q.SEquatableEdge(1,2)]);assert(g.ContainsEdge(new Q.SEquatableEdge(1,2)));assert(g.ContainsEdge(2,1));});
test('GraphExtensionsTests.ToUndirectedGraph_VertexPairs_Throws',()=>assert.throws(()=>Q.ToUndirectedGraph(null)));

// Upstream GraphTestsBase.AddVertex_EquatableVertex_Test / ContainsVertex_EquatableVertex_Test.
class EquatableVertex {
 constructor(id){this.Id=id;}
 Equals(other){return other instanceof EquatableVertex&&this.Id===other.Id;}
 GetHashCode(){return this.Id;}
 toString(){return String(this.Id);}
}
for(const C of [Q.AdjacencyGraph,Q.BidirectionalGraph,Q.UndirectedGraph,Q.ClusteredAdjacencyGraph])test(`${C.name}Tests.AddVertex_EquatableVertex`,()=>{const g=C===Q.ClusteredAdjacencyGraph?new C(new Q.AdjacencyGraph()):new C(),a=new EquatableVertex(1),same=new EquatableVertex(1),b=new EquatableVertex(2);assert(g.AddVertex(a));assert(!g.AddVertex(same));assert(g.AddVertex(b));assert.equal(g.VertexCount,2);assert.equal(g.Vertices[0],a);});
for(const name of ['AdjacencyGraph','BidirectionalGraph','UndirectedGraph','ArrayAdjacencyGraph','ArrayBidirectionalGraph','ArrayUndirectedGraph','CompressedSparseRowGraph','UndirectedBidirectionalGraph','BidirectionalAdapterGraph','ClusteredAdjacencyGraph','EdgeListGraph','ReversedBidirectionalGraph'])test(`${name}Tests.ContainsVertex_EquatableVertex`,()=>{
 const base=new Q.BidirectionalGraph(),a=new EquatableVertex(1),b=new EquatableVertex(2);base.AddVerticesAndEdge(new Q.Edge(a,b));let g;
 if(name==='AdjacencyGraph'||name==='BidirectionalGraph')g=new Q[name](base);
 else if(name==='UndirectedGraph')g=Q.ToUndirectedGraph(base);
 else if(name==='EdgeListGraph'){g=new Q.EdgeListGraph();g.AddEdge(base.Edges[0]);}
 else if(name==='ArrayUndirectedGraph')g=new Q.ArrayUndirectedGraph(Q.ToUndirectedGraph(base));
 else g=new Q[name](base);
 assert(g.ContainsVertex(new EquatableVertex(1)));assert(g.ContainsVertex(new EquatableVertex(2)));assert(!g.ContainsVertex(new EquatableVertex(3)));if(g.OutEdges)assert.equal(g.OutEdges(new EquatableVertex(1)).length,name==='ReversedBidirectionalGraph'?0:1);
});
test('Graph equatable vertex endpoints preserve edge identity',()=>{const g=new Q.BidirectionalGraph(),a=new EquatableVertex(1),b=new EquatableVertex(2),edge=new Q.Edge(new EquatableVertex(1),new EquatableVertex(2));g.AddVertexRange([a,b]);g.AddEdge(edge);assert.equal(g.VertexCount,2);assert.equal(g.OutEdge(a,0),edge);assert.equal(g.InEdge(b,0),edge);assert(g.ContainsEdge(new EquatableVertex(1),new EquatableVertex(2)));assert(g.RemoveVertex(new EquatableVertex(1)));assert.equal(g.EdgeCount,0);});
test('EqualityMap/EqualitySet handle hash collisions, undefined, NaN and reference keys',async()=>{const {EqualityMap,EqualitySet}=await import('../src/equality.js');class Key{constructor(id){this.id=id;}Equals(k){return k instanceof Key&&k.id===this.id;}GetHashCode(){return 0;}}const a=new Key(1),b=new Key(2),map=new EqualityMap([[a,10],[b,20],[undefined,30],[NaN,40]]);map.set(new Key(1),11);assert.equal(map.size,4);assert.equal(map.get(new Key(1)),11);assert.equal(map.get(new Key(2)),20);assert.equal(map.get(undefined),30);assert.equal(map.get(NaN),40);assert.equal([...map.keys()][0],a);assert(map.delete(new Key(1)));assert.equal(map.get(new Key(1)),undefined);const set=new EqualitySet([a,b,new Key(1),undefined,NaN]);assert.equal(set.size,4);assert(set.has(new Key(1)));assert(set.delete(new Key(1)));assert(!set.has(a));const x={},y={};map.set(x,1);assert(!map.has(y));set.add(x);assert(!set.has(y));map.clear();set.clear();assert.equal(map.size,0);assert.equal(set.size,0);});
test('EqualityMap supports Equals without hash override',async()=>{const {EqualityMap}=await import('../src/equality.js');class Key{constructor(id){this.id=id;}Equals(x){return x instanceof Key&&this.id===x.id;}}const map=new EqualityMap([[new Key(1),'a'],[new Key(2),'b']]);assert.equal(map.get(new Key(2)),'b');map.set(new Key(1),'c');assert.equal(map.size,2);assert.equal(map.get(new Key(1)),'c');});
test('EqualitySet composition and queries preserve explicit equality with native Set operands',async()=>{const {EqualitySet}=await import('../src/equality.js');const a=new EquatableVertex(1),b=new EquatableVertex(2),c=new EquatableVertex(3),left=new EqualitySet([a,b]),right=new Set([new EquatableVertex(2),c]);const ids=set=>[...set].map(v=>v.Id);assert.deepEqual(ids(left.union(right)),[1,2,3]);assert.equal([...left.union(right)][1],b);assert.deepEqual(ids(left.intersection(right)),[2]);assert.deepEqual(ids(left.difference(right)),[1]);assert.deepEqual(ids(left.symmetricDifference(right)),[1,3]);assert(left.isSubsetOf(new Set([new EquatableVertex(1),new EquatableVertex(2),c])));assert(!left.isSubsetOf(right));assert(left.isSupersetOf(new Set([new EquatableVertex(1)])));assert(!left.isSupersetOf(right));assert(!left.isDisjointFrom(right));assert(left.isDisjointFrom(new Set([c])));assert.equal(left.size,2);const setLike={size:2,has:v=>v===2||v===3,*keys(){yield 2;yield 3;}};assert.deepEqual([...new EqualitySet([1,2]).union(setLike)],[1,2,3]);for(const value of [null,[],{size:-1,has(){},keys(){}},{size:1,has:0,keys(){}}])assert.throws(()=>left.union(value));});

// Upstream Events/*Tests constructor fixtures preserve object identity and edge orientation.
test('VertexEventArgsTests.Constructor',()=>{const vertex={Id:'1'},args=new Q.VertexEventArgs(vertex);assert.equal(args.Vertex,vertex);});
test('VertexEventArgsTests.Constructor_Throws',()=>assert.throws(()=>new Q.VertexEventArgs(null),Q.ArgumentNullException));
test('EdgeEventArgsTests.Constructor',()=>{const edge=new Q.Edge(1,2),args=new Q.EdgeEventArgs(edge);assert.equal(args.Edge,edge);});
test('EdgeEventArgsTests.Constructor_Throws',()=>assert.throws(()=>new Q.EdgeEventArgs(null),Q.ArgumentNullException));
test('UndirectedEdgeEventArgsTests.Constructor',()=>{const edge=new Q.Edge(1,2);for(const reversed of [false,true]){const args=new Q.UndirectedEdgeEventArgs(edge,reversed);assert.equal(args.Edge,edge);assert.equal(args.Reversed,reversed);assert.equal(args.Source,reversed?2:1);assert.equal(args.Target,reversed?1:2);}});
test('UndirectedEdgeEventArgsTests.Constructor_Throws',()=>{for(const reversed of [false,true])assert.throws(()=>new Q.UndirectedEdgeEventArgs(null,reversed),Q.ArgumentNullException);});
test('ExceptionTests.ExceptionsConstructor',()=>{for(const name of ['NegativeCapacityException','NegativeCycleGraphException','NegativeWeightException','NonAcyclicGraphException','NonStronglyConnectedGraphException','NoPathFoundException','ParallelEdgeNotAllowedException','VertexNotFoundException']){const inner=new Error('Inner'),message='Test exception message.',error=new Q[name](message,inner),withoutInner=new Q[name](message,null);assert.equal(error.Message,message);assert.equal(error.message,message);assert.equal(error.InnerException,inner);assert.equal(error.cause,inner);assert.equal(withoutInner.Message,message);assert.equal(withoutInner.InnerException,null);assert.equal(error.name,name);assert(error instanceof Error);assert.equal(new Q[name](message,{cause:inner}).InnerException,inner);}});

// Upstream Graphs/{DegreeTests,ArrayGraphTests} run the original graph factory corpus.
let graphFactoryCorpus;
function originalGraphFactoryCorpus(){if(graphFactoryCorpus)return graphFactoryCorpus;const directory=new URL('./fixtures/GraphML/',import.meta.url),files=readdirSync(directory).filter(name=>/^g\.\d+\.\d+\.graphml$/.test(name)||name==='DCT8.graphml');assert(files.length>1200);graphFactoryCorpus=[{vertices:[],pairs:[]},{vertices:['1'],pairs:[['1','1']]}];for(const name of files){const xml=readFileSync(new URL(name,directory),'utf8');graphFactoryCorpus.push({vertices:[...xml.matchAll(/<node\b[^>]*\bid="([^"]+)"/g)].map(m=>m[1]),pairs:[...xml.matchAll(/<edge\b[^>]*\bsource="([^"]+)"[^>]*\btarget="([^"]+)"/g)].map(m=>[m[1],m[2]])});}return graphFactoryCorpus;}
function corpusGraph(C,fixture){const graph=new C();graph.AddVertexRange(fixture.vertices);graph.AddVerticesAndEdgeRange(fixture.pairs.map(([a,b])=>new Q.Edge(a,b)));return graph;}
for(const [method,C,degree,factor]of [['DegreeSumEqualsTwiceEdgeCount',Q.BidirectionalGraph,'Degree',2],['InDegreeSumEqualsEdgeCount',Q.BidirectionalGraph,'InDegree',1],['OutDegreeSumEqualsEdgeCount',Q.BidirectionalGraph,'OutDegree',1],['AdjacentDegreeSumEqualsTwiceEdgeCount',Q.UndirectedGraph,'AdjacentDegree',2]])test(`DegreeTests.${method}`,()=>{for(const fixture of originalGraphFactoryCorpus()){const graph=corpusGraph(C,fixture);assert.equal(graph.Vertices.reduce((sum,v)=>sum+graph[degree](v),0),factor*graph.EdgeCount);if(degree==='OutDegree'){const adjacency=corpusGraph(Q.AdjacencyGraph,fixture);assert.equal(adjacency.Vertices.reduce((sum,v)=>sum+adjacency.OutDegree(v),0),adjacency.EdgeCount);}}});
test('ArrayGraphTests.ConversionToArrayGraph',()=>{for(const fixture of originalGraphFactoryCorpus())for(const [C,Snapshot,edges]of [[Q.AdjacencyGraph,Q.ArrayAdjacencyGraph,'OutEdges'],[Q.BidirectionalGraph,Q.ArrayBidirectionalGraph,'OutEdges'],[Q.UndirectedGraph,Q.ArrayUndirectedGraph,'AdjacentEdges']]){const graph=corpusGraph(C,fixture),array=new Snapshot(graph);assert.equal(array.VertexCount,graph.VertexCount);assert.equal(array.EdgeCount,graph.EdgeCount);assert.deepEqual(array.Vertices,graph.Vertices);assert.deepEqual(array.Edges,graph.Edges);for(const v of graph.Vertices){assert.deepEqual(array[edges](v),graph[edges](v));if(C===Q.BidirectionalGraph)assert.deepEqual(array.InEdges(v),graph.InEdges(v));}}});
test('Equivalent edge hashes support endpoint and tag Equals without GetHashCode',async()=>{const {EqualityMap,EqualitySet}=await import('../src/equality.js');class Key{constructor(id){this.id=id;}Equals(x){return x instanceof Key&&this.id===x.id;}}for(const C of [Q.EquatableEdge,Q.SEdge,Q.SEquatableEdge,Q.EquatableTaggedEdge,Q.STaggedEdge,Q.SEquatableTaggedEdge,Q.EquatableUndirectedEdge,Q.SUndirectedEdge,Q.STaggedUndirectedEdge]){const a=new C(new Key(1),new Key(2),new Key(3)),b=new C(new Key(1),new Key(2),new Key(3));assert(a.Equals(b));assert.equal(a.GetHashCode(),b.GetHashCode());assert.equal(new EqualityMap([[a,1]]).get(b),1);assert.equal(new EqualitySet([a,b]).size,1);const r1=new Q.SReversedEdge(a),r2=new Q.SReversedEdge(b);assert(r1.Equals(r2));assert.equal(r1.GetHashCode(),r2.GetHashCode());}});
